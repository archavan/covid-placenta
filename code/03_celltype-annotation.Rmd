...
## Other lymphocyte clusters: 05, 09, 10, 14, 30, 32
### Notes
...
2. Clusters 09 and 32 are most similar to Tcells from both vento and surya. Given the expression of CD3 genes, that appears to be the correct assignment. 
     ...
     + These clusters contain cells from both decidua and villi samples, but it makes sense for T cells to be of maternal origin. 
...
4. Clusters 10, 14, and 30 are NK cells based on the expression of NKG7 and NCAM1. 
     ...
     + Cluster 30 in unambiguously of decidual sample origin. Clusters 10 and 14 are mixed but makes sense for them to be of maternal origin. 
     
```{r}
annotation$annotation[annotation$cluster %in% c("clust_05")] <- "dec.Tcell_1"
annotation$annotation[annotation$cluster %in% c("clust_09")] <- "dec.Tcell_2"
annotation$annotation[annotation$cluster %in% c("clust_10")] <- "dec.NK_1"
annotation$annotation[annotation$cluster %in% c("clust_14")] <- "dec.NK_2"
annotation$annotation[annotation$cluster %in% c("clust_30")] <- "dec.NK_3"
annotation$annotation[annotation$cluster %in% c("clust_32")] <- "dec.Tcell_3"
...
annotation$annotation[annotation$cluster %in% c("clust_03")] <- "dec.Mono_1"
annotation$annotation[annotation$cluster %in% c("clust_08")] <- "dec.Mono_2"
annotation$annotation[annotation$cluster %in% c("clust_11")] <- "dec.APC_1"
annotation$annotation[annotation$cluster %in% c("clust_12")] <- "dec.APC_2"
annotation$annotation[annotation$cluster %in% c("clust_26")] <- "vil.Ery"
annotation$annotation[annotation$cluster %in% c("clust_28")] <- "dec.Gran"
...
```{r}
seur@meta.data$annotation <- annotation$annotation[match(x = seur@meta.data$seurat_clusters,
                                                         table = annotation$cluster)]

Idents(seur) <- seur@meta.data$annotation

# write seurat object
saveRDS(seur, file = "../results/02_annotation/seurat-object_annotated.rds")
```
